...
export function Header({ user, onOpenAuth, onOpenWatchlist, onShowRestricted, onOpenDetail, onGoHome }: any) {
  ...
  return (
    <header className={`fixed top-0 w-full z-[3000] transition-all duration-300 flex items-center justify-between px-4 md:px-16 py-3 md:py-4 gap-4 ${scrolled ? 'bg-[#050a10]/95 shadow-[0_4px_30px_rgba(0,0,0,0.4)]' : 'bg-[#050a10]/85 backdrop-blur-md border-b border-white/10'}`}>
      <a 
        href="/" 
      ...
      <div className="flex items-center gap-2 md:gap-6 flex-1 justify-end">
        {/* Search Bar */}
        <div className={`relative transition-all duration-300 ${isSearchExpanded ? 'flex-1 max-w-full' : 'w-10 md:w-64 lg:w-80'}`} ref={searchRef}>
          <div 
            className={`search-bar-focus-within bg-[#1e2d46]/60 border border-white/10 rounded-full flex items-center gap-2.5 transition-all duration-300 shadow-[0_4px_15px_rgba(0,0,0,0.2)] ${isSearchExpanded ? 'px-4 py-2' : 'p-2 md:px-4 md:py-2.5'}`}
            onClick={() => {
              if (window.innerWidth < 768) setIsSearchExpanded(true);
            }}
          >
            <Search className="text-text-secondary w-5 h-5 md:w-4 md:h-4 cursor-pointer shrink-0" />
            <input 
              type="text" 
              placeholder="Search movies & TV..." 
              className={`bg-transparent border-none text-white outline-none w-full text-sm placeholder:text-text-secondary/70 transition-all duration-300 ${isSearchExpanded ? 'block' : 'hidden md:block'}`}
              value={searchQuery}
            ...
            {isSearchExpanded && (
              <CloseIcon 
                className="text-text-secondary w-4 h-4 cursor-pointer md:hidden" 
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="relative">
              ...
              {showDropdown && (
                <div className="absolute top-[calc(100%+0.8rem)] right-0 w-[240px] md:w-[280px] max-w-[90vw] bg-modal-bg backdrop-blur-xl border border-white/10 rounded-2xl p-2 z-[150] shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-slideDown origin-top-right">
                  ...
                  <button 
                    className="w-full flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all duration-200 text-text-primary hover:bg-accent/10 hover:border-l-4 hover:border-l-accent hover:pl-[calc(0.75rem-4px)] bg-transparent border-none"
                    onClick={() => {
                  ...
                  <button 
                    className="w-full flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all duration-200 text-[#ff6b6b] hover:bg-[#ff6b6b]/10 hover:border-l-4 hover:border-l-[#ff6b6b] hover:pl-[calc(0.75rem-4px)] border-t border-white/10 mt-2 bg-transparent border-none"
                    onClick={() => {
                      setShowDropdown(false);
                      handleLogout();
                    }}
        ...
        {/* Mobile Menu Toggle */}
        <button 
          className={`md:hidden p-2 rounded-full bg-glass-bg border border-white/10 text-white transition-all duration-300 ${isSearchExpanded ? 'hidden' : 'block'}`}
      ...
      {isMenuOpen && (
        <div 
          ref={mobileMenuRef}
          className="absolute top-[calc(100%+0.5rem)] left-4 right-4 bg-modal-bg backdrop-blur-xl border border-white/10 rounded-2xl p-4 z-[150] shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-slideDown md:hidden"
        >
